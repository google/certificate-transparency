...
TEST_F(CertTest, LoadValidFromDer) {
  Cert leaf(leaf_pem_);
  string der;
  ASSERT_EQ(util::Status::OK, leaf.DerEncoding(&der));
  Cert second;
...
TEST_F(CertTest, DerEncodedNames) {
  ...
  EXPECT_EQ(util::Status::OK, leaf.DerEncodedSubjectName(&leaf_subject));
  EXPECT_FALSE(leaf_subject.empty());

  EXPECT_EQ(util::Status::OK, leaf.DerEncodedIssuerName(&leaf_issuer));
  EXPECT_FALSE(leaf_issuer.empty());

  EXPECT_EQ(util::Status::OK, ca.DerEncodedSubjectName(&ca_subject));
  EXPECT_FALSE(ca_subject.empty());

  EXPECT_EQ(util::Status::OK, ca.DerEncodedIssuerName(&ca_issuer));
...
TEST_F(TbsCertificateTest, DerEncoding) {
  ...
  EXPECT_EQ(util::Status::OK, leaf.DerEncodedTbsCertificate(&cert_tbs_der));
