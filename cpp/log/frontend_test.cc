...
TYPED_TEST(FrontendTest, TestSubmitValid) {
  ...
  ASSERT_EQ(util::Status::OK, cert.Sha256Digest(&sha256_digest));
  EXPECT_TRUE(
  ...
  string der_string;
  ASSERT_EQ(util::Status::OK, cert.DerEncoding(&der_string));
...
TYPED_TEST(FrontendTest, TestSubmitValidWithIntermediate) {
  ...
  ASSERT_EQ(util::Status::OK, cert2.DerEncoding(&der_string));
  EXPECT_EQ(H(der_string),
...
TYPED_TEST(FrontendTest, TestSubmitPrecert) {
  ...
  ASSERT_EQ(util::Status::OK, pre.DerEncoding(&pre_der));
  ASSERT_EQ(util::Status::OK, ca.DerEncoding(&ca_der));
...
TYPED_TEST(FrontendTest, TestSubmitPrecertUsingPreCA) {
  ...
  ASSERT_EQ(util::Status::OK, pre.DerEncoding(&pre_der));
  ASSERT_EQ(util::Status::OK, ca.DerEncoding(&ca_der));
  ASSERT_EQ(util::Status::OK, ca_pre.DerEncoding(&ca_pre_der));
