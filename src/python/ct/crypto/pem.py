...
class PemReader(object):
    ...
    def __init__(self, fileobj, markers, skip_invalid_blobs=True):
        ...
        Args:
            ...
            markers:             an iterable of markers accepted by the reader,
                                 e.g., CERTIFICATE, RSA PUBLIC KEY, etc.
    ...
    def __iter__(self):
        """Returns a generator function that yields decoded (blob, marker)
        tuples."""
    ...
    def from_file(cls, pem_file, markers, skip_invalid_blobs=True):
        """Create a PemReader for reading a file. Caller is responsible for
    ...
    def read_blocks(self):
        """Reads the next PEM blob.
        Returns: (raw_string, marker) tuples containing the decoded blob and the
        ...
        while not self.__eof:
            ...
            if not marker:
                self.__eof = True
                if not self.__skip_invalid_blobs and not self.__valid_blobs_read:
                    raise PemError("No PEM header")
                raise StopIteration

            ret = ""
            footer = _END_TEMPLATE % self.__marker_dict[line]
            footer_found = False
            ...
            # TODO(ekasper): determine if this assumption is overly strict, i.e.,
            # whether blocks such as BEGIN RSA PUBLIC KEY...END PUBLIC KEY are
...
class PemWriter(object):
    ...
    def __init__(self, fileobj, marker):
        ...
        Args:
            ...
            reading, and must be positioned at the writing position. Rather than
            initializing directly from a file object, it is recommended to use
            the from_file() constructor.
            marker:  the marker to use in headers.
            mode: a mode hint for file objects that have no mode attribute.
            Used when constructing from a StringIO. Do not set this unless you
    ...
    def from_file(cls, filename, marker, append=False):
        """Construct a writer for writing to a file. Caller is responsible for
        closing the writer afterwards.
        Args:
    ...
    def write(self, blob, check_newline=True):
        ...
        Args:
            blob:          a binary blob.
            check_newline: if True, check whether the current position is
                           at the beginning of a new line and add a newline
                           if not.
        Raises:
    ...
    def pem_string(cls, blob, marker):
        ...
        Args:
            ...
            marker: the marker to use in headers/footers.
        Returns: a string of concatenated PEM blobs."""
...
def from_pem(pem_string, markers):
    """Read a single PEM blob from a string. Ignores everything before and after
    the first blob with valid markers.
    Args:
        pem_string: the PEM string
        markers: a single marker string or an iterable containing
                 all accepted markers, such as CERTIFICATE, RSA PUBLIC KEY,
                 PUBLIC KEY, etc.
    Returns:
...
def from_pem_file(pem_file, markers):
    """Read a single PEM blob from a file. Ignores everything before and after
    the first blob with valid markers.
    Args:
...
def pem_blocks(pem_string, markers, skip_invalid_blobs=True):
    ...
    Args:
        pem_string:         the PEM string
        markers:            a single marker string or an iterable containing
                            all accepted markers, such as CERTIFICATE,
                            RSA PUBLIC KEY, PUBLIC KEY, etc.
        skip_invalid_blobs: if False, invalid PEM blobs cause a PemError.
                            If True, invalid blobs are skipped. In
                            non-skip mode, an immediate StopIteration
                            before any valid blocks are found, also
...
def blocks_to_pem_file(blobs, filename, marker):
    """Convert binary blobs of one and the same type to PEM format and write
    to file.
    Args:
        blobs:     an iterable of binary blobs
        filename: the file to write to
