...
class LogVerifierTest(object):
    def test_verify_sth(self):
        verifier = verify.LogVerifier(self.default_key_info)
        self.assertTrue(verifier.verify_sth(self.default_sth))

    def test_verify_sth_fails_for_bad_signature(self):
        verifier = verify.LogVerifier(self.default_key_info)
        default_sth = self.default_sth
...
class LogVerifierRsaTest(LogVerifierTest, unittest.TestCase):
    ...
    default_sth.tree_size = 1130
    default_sth.timestamp = 1442500998291
    ...
    default_key_info.type = client_pb2.KeyInfo.RSA
