...
public class CertificateData implements Serializable, Comparable<CertificateData> {
	...
	public static CertificateData newData(LogEntryType type, X509CertificateObject certificate, TBSCertificate preCertificate, byte[] extensions, List<X509CertificateObject> extraCertificates) {
		Preconditions.checkArgument((type == LogEntryType.X509_ENTRY && certificate != null || type == LogEntryType.PRECERT_ENTRY && preCertificate != null) && (preCertificate == null ^ certificate == null),
				"The certificate cannot be null for X509 entry types or the preCertificate cannot be null for pre-certificate entry types.");
		return new CertificateData(type, certificate, preCertificate, extensions, extraCertificates);
	}

	protected CertificateData(LogEntryType type, X509CertificateObject certificate, TBSCertificate preCertificate, byte[] extensions, List<X509CertificateObject> extraCertificates) {
		this.type = type;
	...
	public int compareTo(CertificateData other) {
		if (type != other.getType())
			return type.getIndex() - other.getType().getIndex();
