...
public class CertificateData implements Serializable, Comparable<CertificateData> {
   ...
   */
  public static CertificateData newData(LogEntryType type, X509CertificateObject certificate,
      TBSCertificate preCertificate, byte[] extensions,
      List<X509CertificateObject> extraCertificates) {
    Preconditions
        .checkArgument(
            (type == LogEntryType.X509_ENTRY && certificate != null || type == LogEntryType.PRECERT_ENTRY
                && preCertificate != null)
  ...
  protected CertificateData(LogEntryType type, X509CertificateObject certificate,
      TBSCertificate preCertificate, byte[] extensions,
      List<X509CertificateObject> extraCertificates) {
    this.type = type;
