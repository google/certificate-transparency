...
/**
 * The main CT log client. Can upload certificate chains to the ctlog or retrieve entries from it.
 */
public class CTLogClient {
  ...
  private void retrieve(String strStart, String strEnd, String outputFile) throws Exception {
    ...
    int end = all ? getTreeSize() - 1 : strEnd != null ? Integer.parseInt(strEnd) : start + 1;
    Stopwatch timer = Stopwatch.createStarted();
    httpClient.getLogEntries(end - 1, end);
    CertificateDataSet logEntries = httpClient.getLogEntries(start, end);
    timer.stop();
    LOG.info(String.format("Retrieved %d log entries in %s seconds.", logEntries.getDataSet()
        .size(), timer.elapsed(TimeUnit.SECONDS)));

    if (outputFile != null)
      try (ObjectOutput objectOutput =
          new ObjectOutputStream(new FileOutputStream(outputFile, false))) {
