...
func TestX509MerkleTreeLeafHash(t *testing.T) {
	...
	b := new(bytes.Buffer)
	if err := SerializeV1SCTMerkleTreeLeafX509(b, sct.Timestamp, certDER.Bytes); err != nil {
	...
	if !bytes.Equal(b.Bytes(), leaf) {
		t.Errorf("SerializeV1SCTMerkleTreeLeafX509(): got\n %x, want\n%x", b.Bytes(), sctB)
	}
...
func TestJSONMerkleTreeLeaf(t *testing.T) {
	...
	b := new(bytes.Buffer)
	if err := SerializeV1SCTMerkleTreeLeafJSON(b, timestamp, data); err != nil {
		t.Fatalf("Failed to Serialize x509 leaf: %v", err)
	}
	leaf := dh("")

	if !bytes.Equal(b.Bytes(), leaf) {
		t.Errorf("SerializeV1SCTMerkleTreeLeafJSON(): got\n%x, want\n%x", b.Bytes(), leaf)
	}
}
